  ...
  const handleGrant = async (e: React.FormEvent) => {
    ...
    try {
      const response = await axios.post<{ message: string }>(
        `${process.env.NEXT_PUBLIC_API_URL}/admin/grant-free-subscription`,
        {
...
const AdminSettingsPage: React.FC = () => {
  ...
  const [isAddingAdmin, setIsAddingAdmin] = useState<boolean>(true);
  const [maintenanceMode, setMaintenanceMode] = useState<boolean>(false);
  const [adminActivity, setAdminActivity] = useState<FormattedActivity[]>([]);
  const { user } = useAuth();
  ...
  const formatActivity = (activity: Activity): FormattedActivity => {
    return {
      id: activity.activity_id || activity.id || 0,
  ...
  useEffect(() => {
    ...
    const checkAdminStatus = async () => {
      try {
        const statusResponse = await axios.get<{ isAdmin: boolean }>(
    ...
    const fetchMaintenanceStatus = async () => {
      try {
        const response = await axios.get<{ maintenanceMode: boolean }>(
          `${process.env.NEXT_PUBLIC_API_URL}/admin/system-settings`
        );
  ...
  const handleAdminAction = async (e: React.FormEvent) => {
    ...
    try {
      const response = await axios.post<{ message: string }>(
        `${process.env.NEXT_PUBLIC_API_URL}/admin/${action}-admin`,
        {
  ...
  const handleMaintenanceToggle = async () => {
    try {
      ...
      if (response.status === 200) {
        ...
        const currentState = await axios.get<{ maintenanceMode: boolean }>(
          `${process.env.NEXT_PUBLIC_API_URL}/admin/system-settings`
        );
  ...
  const openActivityDetails = (activity: FormattedActivity) => {
