...
import '@/styles/admin/EnhancedAnalytics.css';
import {
  CircularProgress,
  Table,
  TableBody,
...
interface AnalyticsData {
  ...
  topPages: PageData[];
  userEngagement: {
    newUsersDaily: NewUserData[];
    activeUsers24h: number;
    quizzesTaken: number;
    mostAttemptedLessons: LessonData[];
  };
  environmentInfo: {
...
const Analytics: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'basic' | 'users' | 'environment'>('basic');
              ...
              <div className="analytics-controls">
                ...
                <div style={{ minWidth: 160 }}>
                  <Select
                    ...
                    onChange={(opt) => opt && setTimeRange(opt.value)}
                    options={[
                      { value: '7days', label: 'Last 7 days' },
                      { value: '30days', label: 'Last 30 days' },
                      { value: '90days', label: 'Last 90 days' },
                                ...
                                {analytics.environmentInfo?.topCountries?.map((country, index) => (
                                  <TableRow key={index}>
                                    ...
                                    <TableCell align="right">
                                      {(
                                        (parseInt(country.visits) / (analytics.visits?.total || 1)) *
                                        100
                                      ).toFixed(1)}
                                      %
                                    </TableCell>
