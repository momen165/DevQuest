...
interface UserscountMap {
  [key: number]: number;
}

interface EnrollmentsMap {
  [key: number]: boolean;
}
...
const CoursesPage: React.FC = () => {
  ...
  const { user } = useAuth();
  const [userscount, setUserscount] = useState<UserscountMap>({});
  ...
  useEffect(() => {
    const fetchOptimizedCoursesData = async () => {
      ...
      try {
        ...
        const userscountMap: UserscountMap = {};
        const enrollmentsMap: EnrollmentsMap = {};
  ...
  const handleSearch = (term: string) => {
    ...
    const searchTermLower = typeof term === 'string' ? term.toLowerCase() : '';
          ...
          <CourseCard
            ...
            progress={
              ...
              Array.isArray(progress)
                ? (progress as any).find((p: any) => p.course_id === course.course_id)?.progress || 0
