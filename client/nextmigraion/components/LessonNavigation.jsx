...
import { useAuth } from '@/contexts/AuthContext'; // Assuming this context provides user and token
...
const LessonNavigation = ({
    ...
    const fetchMenuData = async () => {
      ...
      try {
        ...
        const sectionResponse = await axios.get(
          `${process.env.NEXT_PUBLIC_API_URL}/sections/${currentSectionId}`,
          {
            headers: {
        ...
        if (sectionResponse.status === 200 && sectionResponse.data) {
          ...
          const courseResponse = await axios.get(
            `${process.env.NEXT_PUBLIC_API_URL}/courses/${fetchedCourseId}`,
            {
              headers: {
                Authorization: `Bearer ${user.token}`,
              },
          ...
          const sectionsResponse = await axios.get(
            `${process.env.NEXT_PUBLIC_API_URL}/sections/course/${fetchedCourseId}?user_id=${user.user_id}`, // Pass user_id to get completion status
            {
              headers: {
                Authorization: `Bearer ${user.token}`,
              },
  ...
  useEffect(() => {
    ...
    if (!accessible && !(sectionIndex === 0 && lessonIndex === 0)) {
      ...
      router.push(`/lesson/${redirectLessonId}`);
    }
  ...
  const completeLesson = async () => {
    ...
    try {
      const response = await axios.put(
        `${process.env.NEXT_PUBLIC_API_URL}/update-lesson-progress`,
        {
  ...
  const goToPreviousLesson = () => {
    if (lessonIndexInSection > 0) {
      router.push(`/lesson/${currentSectionLessonsForNav[lessonIndexInSection - 1].lesson_id}`);
      ...
      router.push(`/lesson/${lastLessonInPreviousSection.lesson_id}`);
    }
  };
  ...
  const goToNextLesson = () => {
    ...
    if (lessonIndexInSection < currentSectionLessonsForNav.length - 1) {
      ...
      onNext(); 
      router.push(`/lesson/${firstLessonInNextSection.lesson_id}`);
    } else {
      showNotification(
        'success',
