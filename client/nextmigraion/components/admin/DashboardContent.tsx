...
import "@/styles/admin/DashboardContent.css";
...
const DashboardContent: React.FC = () => {
  ...
  const [newStudentsList, setNewStudentsList] = useState<Student[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        if (!token) {
          // We rely on useAuth to handle redirection if not logged in,
          // but here we stop fetching to avoid errors.
          return;
        }
        ...
        const studentsResponse = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/students`, { headers });
        ...
        const coursesResponse = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/courses`, { headers });
        setCoursesCount(coursesResponse.data.length);

        const activityResponse = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/activities/recent`, {
          headers,
        });
        setAllActivities(activityResponse.data);
        setRecentActivity(activityResponse.data.slice(0, 5));

        const feedbackResponse = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/feedback/recent`, {
          headers,
        });
        setRecentFeedback(feedbackResponse.data);

        const ticketsResponse = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/support-tickets/recent`, {
    ...
    <div className="dashboard-content">
        ...
        <header className="dashboard-header">
          ...
          <div className="profile">
            ...
            <Link href="/" className="dashboard-to-homeLink">
              ← Back to website
            </Link>
          </div>
        </header>
        ...
        <div className="dashboard-sidebar">
          ...
          <div className="recent-feedback">
            <div className="activity-header">
              <h2>New Feedback</h2>
              <Link href="/Feedback" className="see-all-button">
                See all
              </Link>
            </div>
            <ul>
          ...
          <div className="recent-tickets">
            <div className="activity-header">
              ...
              <Link href="/Support" className="see-all-button">
                See all
              </Link>
            </div>
            <ul>
