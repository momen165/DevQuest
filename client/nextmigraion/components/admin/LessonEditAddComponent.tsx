...
import '@/styles/admin/LessonEditAddComponent.css';
...
interface LessonEditAddComponentProps {
  section: Section;
  lesson?: Lesson | null;
...
const LessonEditAddComponent: React.FC<LessonEditAddComponentProps> = ({ section, lesson = null, onSave, onCancel, onDelete }) => {
  const { user } = useAuth();

  const getDefaultTestCase = (): TestCase => ({
    input: '',
    expected_output: '',
    auto_detect: false,
    use_pattern: false,
    ...
    if (lesson?.test_cases) {
      try {
        const cases = Array.isArray(lesson.test_cases)
          ? lesson.test_cases
          : JSON.parse(lesson.test_cases as unknown as string);
        return cases.map((test: any) => ({
          input: test.input || '',
    ...
    const fetchCourseLanguage = async () => {
      ...
      try {
        const sectionResponse = await axios.get(
          `${process.env.NEXT_PUBLIC_API_URL}/sections/${section.section_id}`,
        ...
        const courseResponse = await axios.get(
          `${process.env.NEXT_PUBLIC_API_URL}/courses/${sectionResponse.data.course_id}`,
          { headers: { Authorization: `Bearer ${user?.token}` } }
        );
    ...
    try {
      ...
      if (lesson && lesson.lesson_id) {
        const response = await axios.put(
          `${process.env.NEXT_PUBLIC_API_URL}/lesson/${lesson.lesson_id}`,
          lessonData,
          {
            headers: {
  ...
  const handleDelete = async () => {
    ...
    try {
      await axios.delete(`${process.env.NEXT_PUBLIC_API_URL}/lesson/${lesson.lesson_id}`, {
        headers: { Authorization: `Bearer ${user?.token}` },
      });
      if (onDelete) onDelete(lesson.lesson_id);
    } catch (err) {
