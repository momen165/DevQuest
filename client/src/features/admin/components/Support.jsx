...
const Support = () => {
  ...
  useEffect(() => {
    if (
      ...
      !paginatedTickets.find((t) => t.ticket_id === selectedPreviewId)
    ) {
                ...
                >
                  ...
                  <div className="sw-row-main">
                    ...
                    <p className="sw-ticket-preview">
                      {truncateText(
                        ticket.messages?.[ticket.messages?.length - 1]?.message_content
                      )}
                    </p>
                    <div className="sw-row-meta">
                      <span className={`sw-status-pill ${ticket.status}`}>{ticket.status}</span>
                      {ticket.source && (
                        <span
                          className="sw-ticket-id"
            ...
            <>
              <header className="sw-detail-header">
                ...
                <div className="sw-detail-info-grid">
                  <div className="sw-info-item">
                    <label>Status</label>
                    <span className={`sw-status-pill ${selectedTicket.status}`}>
                  ...
                  <div className="sw-info-item">
                    ...
                    <span>
                      {new Date(selectedTicket.time_opened).toLocaleString([], {
                        dateStyle: "medium",
                        timeStyle: "short",
                      })}
                    </span>
              ...
              <div className="sw-thread">
                {selectedTicket.messages?.map((msg, i) => (
                  <div
                    key={i}
                    className={`sw-message ${msg.sender_type === "admin" ? "admin" : "user"}`}
                  >
                    <div className="sw-message-bubble">{msg.message_content}</div>
                    <div className="sw-message-meta">
                      {msg.sender_type === "admin" ? "Support Agent" : "Customer"} •{" "}
        ...
        <div className="sw-bulk-bar">
          ...
          <div className="sw-header-actions">
            <button
              className="sw-btn sw-btn-ghost"
              style={{ color: "white" }}
              onClick={() => handleBulkAction("solved")}
            >
