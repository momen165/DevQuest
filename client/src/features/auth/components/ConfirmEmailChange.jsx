...
function ConfirmEmailChange() {
  ...
  useEffect(() => {
    const confirmEmailChange = async () => {
      ...
      try {
        const response = await axios.post(`${import.meta.env.VITE_API_URL}/confirmEmailChange`, {
          token,
        });
        setStatus('success');
        ...
        setStatus('error');
        const errorMessage = error.response?.data?.error || 'Failed to confirm email change';
          ...
          <div className="change-password-form-container">
            ...
            {status === 'error' && (
              <div className="confirmation-message error">
                ...
                <p>Please try requesting a new email change from your account settings.</p>
              </div>
            )}
          </div>
        </div>
