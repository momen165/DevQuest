  ...
  try {
    ...
    switch (eventType) {
      case 'code_submission':
        ...
        const hasCodeNoviceBadge = await BadgeModel.userHasBadge(userId, BadgeModel.BadgeType.CODE_NOVICE);
        if (!hasCodeNoviceBadge) {
          badgeAwarded = await BadgeModel.awardBadge(userId, BadgeModel.BadgeType.CODE_NOVICE);
        }
        break;
      ...
      case 'lesson_completed':
        ...
        if (eventData.completedLessonsCount >= 10) {
          const hasLessonSmasherBadge = await BadgeModel.userHasBadge(userId, BadgeModel.BadgeType.LESSON_SMASHER);
          if (!hasLessonSmasherBadge) {
            badgeAwarded = await BadgeModel.awardBadge(userId, BadgeModel.BadgeType.LESSON_SMASHER);
          }
        }
        break;
      ...
      case 'language_used':
        ...
        if (eventData.uniqueLanguagesCount >= 3) {
          const hasLanguageExplorerBadge = await BadgeModel.userHasBadge(userId, BadgeModel.BadgeType.LANGUAGE_EXPLORER);
          if (!hasLanguageExplorerBadge) {
            badgeAwarded = await BadgeModel.awardBadge(userId, BadgeModel.BadgeType.LANGUAGE_EXPLORER);
          }
        }
        break;
      ...
      case 'streak_update':
        ...
        if (eventData.streakDays >= 7) {
          const hasStreakMasterBadge = await BadgeModel.userHasBadge(userId, BadgeModel.BadgeType.STREAK_MASTER);
          if (!hasStreakMasterBadge) {
            badgeAwarded = await BadgeModel.awardBadge(userId, BadgeModel.BadgeType.STREAK_MASTER);
          }
        }
        break;
      ...
      case 'xp_update':
        ...
        if (eventData.totalXp >= 100) {
          const hasXpAchieverBadge = await BadgeModel.userHasBadge(userId, BadgeModel.BadgeType.XP_ACHIEVER);
          if (!hasXpAchieverBadge) {
            badgeAwarded = await BadgeModel.awardBadge(userId, BadgeModel.BadgeType.XP_ACHIEVER);
          }
        }
        break;
    ...
    console.error(`Error checking and awarding badges for ${eventType}:`, error);
    return null;
  }
};
...
const awardBadgeToUser = async (req, res) => {
  try {
    ...
    // Validate badge type
    const validBadgeType = Object.values(BadgeModel.BadgeType).includes(badgeType);
    if (!validBadgeType) {
      return res.status(400).json({
        success: false,
