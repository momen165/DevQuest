...
const updateUserStreak = async (req, res, next) => {
  ...
  try {
    ...
    if (userData?.updated_today) {
      console.log(
        `[Streak Debug] Skipping update - already updated today for user ${userId}`,
      );
      return next();
    }
    ...
    if (rows.length > 0) {
      ...
      if (visited_yesterday) {
        ...
        newStreak += 1;
        console.log(
          `[Streak Debug] Incrementing streak for user ${userId} from ${oldStreak} to ${newStreak} - visited yesterday`,
        );
      } else {
        // If they didn't visit yesterday, reset streak to 1
        newStreak = 1;
        console.log(
          `[Streak Debug] Resetting streak for user ${userId} from ${oldStreak} to ${newStreak} - didn't visit yesterday`,
        );
      }
