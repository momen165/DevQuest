...
const { getFeedback, submitFeedback, getCoursesWithRatings, replyToFeedback, getPublicFeedback, checkFeedbackEligibility, getRecentFeedback, reopenFeedback } = require('../controllers/feedback.controller');
...
router.get('/feedback/eligibility/:courseId', authenticateToken, async (req, res) => {
    try {
        const eligibility = await checkFeedbackEligibility(req.user.userId, req.params.courseId);
